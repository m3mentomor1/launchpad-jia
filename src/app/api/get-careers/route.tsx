// src/app/api/get-careers/route.tsx
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const authUserRole = await db
      .collection("members")
      .findOne({ email: userEmail, orgID });
    // Filter careers based on the user's role
    const filter: any = { orgID };
    if (
      authUserRole?.role === "hiring_manager" &&
      authUserRole?.careers?.length > 0
    ) {
    ...
    if (sortConfig) {
      ...
      defaultSort = {
        [key]: config.direction === "ascending" ? 1 : -1,
        _id: -1,
      };
    }
                    ...
                    $and: [
                      {
                        $or: [
                          ...
                          {
                            $eq: [
                              { $type: "$interviews.applicationStatus" },
                              "missing",
                            ],
                          },
                        ],
