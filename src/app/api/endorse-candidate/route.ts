// src/app/api/endorse-candidate/route.tsx
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await db
      .collection("interviews")
      .updateOne({ _id: new ObjectId(uid) }, { $set: { ...update } });
    ...
    return NextResponse.json({
      message: "Candidate endorsed",
      updatedInterview: { ...interview, ...update },
    });
  } catch (error) {
    return NextResponse.json(
      { error: "Failed to endorse candidate" },
      { status: 500 }
    );
  }
}
