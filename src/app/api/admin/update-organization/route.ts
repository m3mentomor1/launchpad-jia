// src/app/api/admin/update-organization/route.tsx
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (members) {
      ...
      const uniqueEmails = [
        ...new Set(members.map((member: any) => member.email)),
      ];
      ...
      if (removedMembers?.length > 0) {
        await db
          .collection("members")
          .deleteMany({ _id: { $in: removedMembers.map((m) => m._id) } });
      }
      // New members
      ...
      // Updated members
      const updatedMembers = existingMembers.filter((m) =>
        ...
        await db.collection("members").bulkWrite(
          updatedMembers.map((m) => {
            const updatedMember = members.find(
              (member: any) => member.email === m.email
            );
            return {
              updateOne: {
                filter: { _id: m._id },
