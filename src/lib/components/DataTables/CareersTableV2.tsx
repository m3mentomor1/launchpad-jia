    ...
    const fetchOrgDetails = async () => {
      try {
        ...
        setAvailableJobSlots(
          (orgDetails.data?.plan?.jobLimit || 999) +
            (orgDetails.data?.extraJobSlots || 0)
        );
      } catch (error) {
        console.error("Error fetching org details:", error);
        // Set a high default if there's an error
        setAvailableJobSlots(999);
      }
    };
    if (activeOrg) {
      fetchOrgDetails();
  ...
  return (
    <button
      className={`btn btn-sm ${
        sortConfig.key === columnName ? "btn-primary" : "btn-white"
      } mr-1`}
      onClick={() => requestSort(columnName)}
    >
      <i
        className={`la la-sort${
