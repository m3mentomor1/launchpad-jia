...
export default function CandidateMenu({
  ...
  const [isAIInterviewAnalysisOpen, setIsAIInterviewAnalysisOpen] =
    useState(false);
    ...
    const fetchInterviewDetails = async () => {
      ...
      try {
        if (!isLoadingSettingsRef.current) {
          ...
          const interviewDetailsResponse = await axios.post(
          ...
          if (
            interviewDetailsResponse.data?.analysis ||
            interviewDetailsResponse.data?.summary
          ) {
  ...
  useEffect(() => {
    const fetchCVData = async () => {
      try {
        const response = await axios.post("/api/load-user-cv", {
  ...
  return (
    <div className="candidate-side-menu">
      <div className="candidate-side-menu-content">
        ...
        {/* Pre-Screening Answers Section */}
        {interviewDetails?.preScreeningAnswers &&
          Object.keys(interviewDetails.preScreeningAnswers).length > 0 && (
            <>
              <div style={{ marginBottom: "20px" }}>
                ...
                >
                  ...
                  >
                    <i
                      className="la la-clipboard-list"
                ...
                >
                  {interviewDetails.preScreeningQuestions?.map(
                    (question: any, idx: number) => {
                      const answer =
                        interviewDetails.preScreeningAnswers[question.id];
                      ...
                      return (
                        <div
                          ...
                          style={{
                            paddingBottom:
                              idx <
                              interviewDetails.preScreeningQuestions.length - 1
                                ? "12px"
                                : "0",
                            borderBottom:
                              idx <
        ...
        >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <CareerFit
              fit={candidate?.cvStatus ? candidate?.cvStatus : "N/A"}
              assessment={candidate?.cvScreeningReason}
            />
        ...
        {isCVAnalysisOpen && candidate?.cvScreeningReason ? (
          <div>
            <p
              dangerouslySetInnerHTML={{ __html: candidate?.cvScreeningReason }}
            ></p>
          </div>
        ) : (
          <div
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            {analysis?.final_assessment && (
              <CareerFit
                ...
                assessment={extractInterviewAssessment(summary)}
              />
            )}
          ...
          <div>
                      ...
                      analysis?.breakdown?.map((item: any, idx: number) => (
                        ...
                        >
                          ...
                          >
                            ...
                            ></i>
                            <span style={{ fontSize: 14, minWidth: "30%" }}>
                              {item?.key}
                            </span>
                            {/* Progress bar */}
                            <div
                            ...
                            >
                              <div
                                style={{
                                  ...
                                  height: "100%",
                                  borderRadius: 4,
                                  background: breakdownColorScheme[idx],
                                }}
                              />
                            </div>
            ...
            {!summary &&
              ...
              !isGeneratingAnalysis && (
                ...
                >
                  <p
                    style={{ fontSize: 16, color: "#717680", fontWeight: 500 }}
                  >
                    Candidate has not taken AI Interview. <br /> Analysis
                    unavailable.
                  </p>
                </div>
              )}
