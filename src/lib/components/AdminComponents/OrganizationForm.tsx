// src\lib\components\AdminComponents\OrganizationForm.tsx
"use client";
        ...
        if (image) {
          update.image =
            typeof image === "object"
              ? await uploadFile(
                  image as File,
                  `organization/profile-image/${organization._id}.${
                    (image as File).type.split("/")[1]
                  }`
                )
              : image;
        ...
        if (coverImage) {
          update.coverImage =
            typeof coverImage === "object"
              ? await uploadFile(
                  coverImage as File,
                  `organization/cover-image/${organization._id}.${
                    (coverImage as File).type.split("/")[1]
                  }`
                )
              : coverImage;
          ...
          if (typeof businessPermitFile === "object") {
            document = {
              ...
              filePath: await uploadFile(
                businessPermitFile as File,
                `organization/documents/business-permit/${organization._id}.${
                  (businessPermitFile as File).type.split("/")[1]
                }`
              ),
              fileType: (businessPermitFile as File).type.split("/")[1],
      ...
      try {
        ...
        if (response.status === 200) {
          const currentPlan = response.data.find(
            (plan: any) => plan._id === organization?.planId
          );
        ...
        >
          ...
          <button
            ...
            style={{
              width: "fit-content",
              background:
                !isFormValid() || isSavingOrganization ? "#D5D7DA" : "black",
              color: "#fff",
              border: "1px solid #E9EAEB",
              padding: "8px 16px",
              borderRadius: "60px",
              cursor:
                !isFormValid() || isSavingOrganization
                  ? "not-allowed"
                  : "pointer",
              whiteSpace: "nowrap",
            }}
